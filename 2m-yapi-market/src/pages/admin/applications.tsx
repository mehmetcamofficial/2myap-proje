...
export function AdminApplicationsPage() {
  ...
  const del = async (id: number) => {
    ...
    await fetch(`/api/admin/applications/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    load();
  };
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_350px]">
        <div className="border border-[hsl(var(--border))]">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[hsl(var(--border))] text-left text-xs uppercase tracking-wider text-[hsl(var(--muted-foreground))]"><th className="px-4 py-3">Başlık</th><th className="px-4 py-3">Sıra</th><th className="px-4 py-3">Durum</th><th className="px-4 py-3" /></tr></thead>
            <tbody>{apps.sort((a, b) => a.displayOrder - b.displayOrder).map((a) => (
              <tr key={a.id} className="border-b border-[hsl(var(--border))]"><td className="px-4 py-3 font-medium">{a.title}</td><td className="px-4 py-3 text-[hsl(var(--muted-foreground))]">{a.displayOrder}</td><td className="px-4 py-3">{a.published ? '✓' : '—'}</td><td className="px-4 py-3"><button onClick={() => edit(a)} className="mr-3 text-xs font-bold text-[hsl(var(--primary))]">Düzenle</button><button onClick={() => del(a.id)} className="text-xs font-bold text-red-500">Sil</button></td></tr>
            ))}</tbody>
          </table>
        </div>
        <div className="border border-[hsl(var(--border))] p-5 max-h-[80vh] overflow-y-auto">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <input placeholder="İlişkili hizmet slug (boş = bağımsız)" value={form.serviceSlug} onChange={(e) => setForm({ ...form, serviceSlug: e.target.value })} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-2 text-sm" />
            <input placeholder="Ana görsel URL" value={form.primaryImage} onChange={(e) => setForm({ ...form, primaryImage: e.target.value })} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-2 text-sm" />
            ...
            <input placeholder="SEO açıklama" value={form.metaDescription} onChange={(e) => setForm({ ...form, metaDescription: e.target.value })} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-2 text-sm" />
