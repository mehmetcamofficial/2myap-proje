...
export function AdminNavigationPage() {
  ...
  const load = useCallback(() => {
    apiFetch('/api/admin/navigation', { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json())
  ...
  const del = async (id: number) => {
    ...
    const res = await apiFetch(`/api/admin/navigation/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_350px]">
        <div className="border border-[hsl(var(--border))]">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[hsl(var(--border))] text-left text-xs uppercase tracking-wider text-[hsl(var(--muted-foreground))]"><th className="px-4 py-3">Etiket</th><th className="px-4 py-3">URL</th><th className="px-4 py-3">Tür</th><th className="px-4 py-3">Durum</th><th className="px-4 py-3" /></tr></thead>
            <tbody>{items.sort((a, b) => a.displayOrder - b.displayOrder).map((n) => (
              <tr key={n.id} className="border-b border-[hsl(var(--border))]"><td className="px-4 py-3 font-medium">{n.parentId ? '└ ' : ''}{n.label}</td><td className="px-4 py-3 text-[hsl(var(--muted-foreground))] text-xs">{n.href}</td><td className="px-4 py-3 text-xs">{n.type}</td><td className="px-4 py-3">{n.published ? '✓' : '—'}</td><td className="px-4 py-3"><button onClick={() => edit(n)} className="mr-3 text-xs font-bold text-[hsl(var(--primary))]">Düzenle</button><button onClick={() => del(n.id)} className="text-xs font-bold text-red-500">Sil</button></td></tr>
            ))}</tbody>
          </table>
        </div>
        <div className="border border-[hsl(var(--border))] p-5">
          <h2 className="font-display text-lg font-bold">{editing ? 'Düzenle' : 'Yeni Öğe'}</h2>
          <div className="mt-4 grid gap-3">
            ...
            <input placeholder="URL" value={form.href} onChange={(e) => setForm({ ...form, href: e.target.value })} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-2 text-sm" />
            <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-2 text-sm">
              <option value="header">Header</option><option value="footer">Footer</option><option value="dropdown">Dropdown</option><option value="mobile">Mobil</option>
            </select>
            <select value={form.parentId} onChange={(e) => setForm({ ...form, parentId: e.target.value })} className="border border-[hsl(var(--border))] bg-[hsl(var(--background))] px-3 py-2 text-sm">
              <option value="">Üst öğe yok</option>{parentItems.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}
            </select>
